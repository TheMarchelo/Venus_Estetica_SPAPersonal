...
function loadCitas() {
    ...
    db.collection('citas').orderBy('timestamp', 'desc').get().then(snapshot => {
        tbody.innerHTML = '';
        if (snapshot.empty) {
            tbody.innerHTML = '<tr><td colspan="6" class="text-center py-4 text-gray-500">No hay citas.</td></tr>';
            return;
        }
        snapshot.forEach(doc => {
            const d = doc.data();
            ...
            tbody.innerHTML += `
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                         <button onclick="editCita('${doc.id}', '${d.name}', '${d.service}', '${d.date}', '${d.phone}', '${d.status}')" class="text-blue-600 hover:text-blue-900 mr-3"><i class="fas fa-edit"></i></button>
                         <button onclick="deleteItem('citas', '${doc.id}')" class="text-red-600 hover:text-red-900"><i class="fas fa-trash"></i></button>
                    </td>
                </tr>
            `;
        });
    }).catch(err => tbody.innerHTML = `<tr><td colspan="6" class="text-center py-4 text-red-500">Error: ${err.message}</td></tr>`);
}
...
function loadPedidos() {
    ...
    db.collection('pedidos').orderBy('timestamp', 'desc').get().then(snapshot => {
        ...
        snapshot.forEach(doc => {
            ...
            let badgeClass = 'bg-gray-100 text-gray-800';
            if (d.status === 'Completado') badgeClass = 'bg-green-100 text-green-800';

            tbody.innerHTML += `
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${date}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">${d.name}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">₡${d.total}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                         <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${badgeClass}">
                            ${d.status}
                        </span>
...
// Citas
function editCita(id, name, service, date, phone, status) {
    ...
    document.getElementById('modal-fields').innerHTML = `
        <div class="grid grid-cols-2 gap-4">
             ...
             <div><label class="block text-sm text-gray-500">Servicio</label><div class="font-medium">${service}</div></div>
        </div>
        <div class="mt-4">
            <label class="block text-sm font-medium text-gray-700">Estado</label>
        ...
        <input type="hidden" id="f-service" value="${service}">
        <input type="hidden" id="f-date" value="${date}">
        <input type="hidden" id="f-phone" value="${phone}">
     `;
    document.getElementById('edit-id').value = id;
...
async function handleFormSubmit(e) {
    ...
    try {
        ...
        if (fileInput && fileInput.files.length > 0) {
            ...
            imageUrl = await uploadImage(fileInput.files[0], type);
        } else {
        ...
        if (type === 'productos') {
            ...
            data = {
                status: document.getElementById('f-status').value
                // we probably don't want to edit other fields for now as they are historical
            };
        }
